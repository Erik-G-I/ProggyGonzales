package gameState;

import java.awt.Color;
import java.awt.Graphics;

import core.GamePanel;
import core.Panel;

public class GameControls extends GameScreen {
	
	public GameControls(Panel panel) {
		this.panel = panel;
	}

	@Override
	public void draw(Graphics g2) {
		this.setPanel(panel);
		this.setGraphics(g2);
		
		String s1,s2,s3,s4,s5,s6;
		
		if(panel.getLang() == Languages.NORWEGIAN) {
			s1 = "Kontroller";
			s2 = "Pil opp, W, mellombar    -    Hopp";
			s3 = "Venstre pil, A    -    Gå til venstre";
			s4 = "Høyre pil, D    -    Gå til høyre";
			s5 = "ESC    -    Pause";
			s6 = "ESC    -    Tilbake til hovedmeny";
		}
		else {
			s1 = "CONTROLS";
			s2 = "Arrow Up, W, Space    -    Jump";
			s3 = "Arrow Left or A    -    Move left";
			s4 = "Arrow Right or D    -    Move right";
			s5 = "ESC    -    Pause Menu";
			s6 = "ESC to go back to Main Menu";
		}
		
		Color transparentRed = new Color(0, 0, 0, 200);
		g2.setColor(transparentRed);
		g2.fillRect(0, 0, panel.screenWidth, panel.screenHeight);
		
		this.draw(100);
		g2.setColor(Color.BLACK);
		g2.drawString(s1, centerText(s1)-10, panel.tileSize*2);
		g2.setColor(Color.WHITE);
		g2.drawString(s1, centerText(s1), panel.tileSize*2);
		
		this.draw(50);
		g2.setColor(Color.WHITE);
		
		g2.drawString(s2, centerText(s2), panel.tileSize*4);
		
		g2.drawString(s3, centerText(s3), panel.tileSize*5);
		
		
		g2.drawString(s4, centerText(s4), panel.tileSize*6);

		g2.drawString(s5, centerText(s5), panel.tileSize*7);

		g2.drawString(s6, panel.tileSize*2, panel.tileSize*10);
		String s7 = ">";
		g2.drawString(s7, panel.tileSize*2-25, panel.tileSize*10);
		
	}
}

